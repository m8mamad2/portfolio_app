...
export function BentoGridDemo() {
  return (
    <div className="bg-black py-6">
        <h1 className="text-4xl font-bold sm:my-10 2xl:mx-60 xl:mx-10 mx-3 my-16">Projects</h1>
        <BentoGrid className="mx-auto">
          {projects.map((item, i) => (
            item.index === 1 
              ? <BentoGridOneImaageItem 
                  key={i}
                  title={item.title}
...
const BentoGridItem = ({ className, title, description, images, index }: { className?: string; title?: string | React.ReactNode; description?: string | React.ReactNode; images: StaticImageData[]; index: number}) => {
  ...
  return (
    ...
    >
      <div className="flex flex-row justify-center items-center gap-5 flex-1 w-full h-full min-h-[6rem] rounded-xl bg-gradient-to-br from-neutral-200 dark:from-neutral-900 dark:to-neutral-800 to-neutral-100">
        {
          images!.map((e, index)=> <Image key={index} src={e} width={ index === 3 || index === 6 ? 85 : 60 } alt="image"/>)
        }
      </div>
...
const BentoGridOneImaageItem = ({
  ...
  return (
    ...
    >
      <div className="flex flex-row justify-center items-center gap-5 flex-1 w-full h-full min-h-[6rem] rounded-xl bg-gradient-to-br from-neutral-200 dark:from-neutral-900 dark:to-neutral-800 to-neutral-100">
        {
          images!.map((e, index)=> <Image key={index} src={e} width={ 110 } alt="image"/>)
        }
      </div>
      <div className="group-hover/bento:translate-x-2 transition duration-200">
        <IconFileBroken />
